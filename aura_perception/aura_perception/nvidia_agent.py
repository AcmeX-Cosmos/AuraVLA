...
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parent.parent.parent / "aura_bringup" / "config" / "config.yaml"
...
def _camera_frame_age_sec(bridge_paths: Any, *, now: float | None = None) -> float:
    ...
    try:
        ...
        # ``timestamp`` was emitted by older Isaac bridge launchers. Accept it
        # while preferring the canonical key so an existing frame can recover
        # immediately after upgrading the bridge.
        captured_at = float(
            metadata.get("captured_at_unix")
